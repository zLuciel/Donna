    ...
    <header className={`w-screen bg-white sticky top-0 z-10 ${isShadow ? 'shadow-header' : ''}`}>
      ...
      <Link href="/"><Image className="logo-header" src="/LogoDonna.png" width={90} height={90} alt="Donna" /></Link>
        ...
        <span className="flex gap-2 flex-col justify-center items-center"><List className="text-xl" /> <p className="prueba text-xs mt-1 leading-3">Mi Lista</p></span>
          ...
          <span onClick={()=> setCartView(true)} className="flex cursor-pointer gap-2 flex-col justify-center items-center"><span className="relative"><b className="absolute count-cart">{dataProduct.length}</b>  <Cart/></span> <p className="text-xs text-[#2E2E2E] mt-1 font-bold leading-3">S/{getTotalPrice()} </p></span>
