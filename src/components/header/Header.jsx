...
const Header = () => {
  return (
    <header className="w-screen sticky top-0 z-10 ">
      ...
      {/* cabezera principal */}
      <div className="lg:container bg-white lg:mx-auto py-4 px-10  header-grid ">
        ...
        <span className="search-header flex w-full rounded-2xl border border-slate-700 overflow-hidden">
          <input
            className="w-full outline-none px-3 py-1"
            type="text"
            placeholder="Dinos que buscas y te mostraremos las mejores ofertas"
          />
         <span className="bg-slate-300 px-4 flex justify-center items-center"><IoSearch /></span>
        </span>
        <div className="cuenta-header flex gap-2 items-center ">
          <span className="flex gap-2 justify-cent flex gap-2 items-center"><FaUser /> Mi cuenta</span>
          <span className="flex gap-2 justify-center items-center"><FaShoppingCart /> S/10.00</span>
        </div>
      </div>
    </header>
  );
